    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-xl border border-app-strong bg-surface shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-4 border-b border-app">
          ...
          <kbd className="font-mono text-[10px] text-tertiary bg-surface-hover border border-app rounded px-1.5 py-0.5">ESC</kbd>
        </div>
        <div className="max-h-[50vh] overflow-y-auto py-1">
          {filtered.length === 0 ? (
            <div className="py-10 text-center text-sm text-tertiary">{t('noResults')}</div>
          ) : filtered.map((r, i) => (
            <button key={r.slug} onClick={() => { router.push(`/${locale}/resource/${r.slug}`); onClose() }} onMouseEnter={() => setSelectedIdx(i)} className={`w-full flex items-center gap-3 px-4 py-2.5 text-left ${i === selectedIdx ? 'bg-surface-hover' : ''}`}>
              <div className="w-7 h-7 rounded bg-surface-hover border border-app flex items-center justify-center overflow-hidden shrink-0">
                {r.logo_url ? <img src={r.logo_url} alt="" className="w-5 h-5" /> : <span className="text-xs text-primary">{tt(r.name, locale)[0]}</span>}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[13.5px] text-primary truncate">{tt(r.name, locale)}</div>
                <div className="text-[12px] text-tertiary truncate">{tt(r.slogan, locale)}</div>
              </div>
              <span className="text-[10.5px] font-mono text-tertiary uppercase tracking-wider">{r.super_category}</span>
              <ArrowRight className={`w-3.5 h-3.5 shrink-0 ${i === selectedIdx ? 'text-[#F5C518]' : 'text-tertiary'}`} />
            </button>
          ))}
        </div>
        <div className="px-4 py-2 border-t border-app flex items-center gap-4 text-[10.5px] text-tertiary">
          <span className="flex items-center gap-1"><kbd className="font-mono bg-surface-hover px-1 rounded">↑↓</kbd> navigate</span>
          <span className="flex items-center gap-1"><kbd className="font-mono bg-surface-hover px-1 rounded">↵</kbd> open</span>
          <span className="flex items-center gap-1"><CmdIcon className="w-2.5 h-2.5" /><kbd className="font-mono bg-surface-hover px-1 rounded">K</kbd> toggle</span>
        </div>
      </div>
    </div>
  )
