...
export function Newsletter() {
  ...
  return (
    <div id="newsletter" className="relative rounded-2xl border border-app bg-surface p-8 md:p-10 overflow-hidden">
      ...
      <div className="relative max-w-2xl">
        <div className="inline-flex items-center gap-2 mb-3 text-[11px] font-semibold tracking-[0.15em] text-[#F5C518]">
          <Mail className="w-3.5 h-3.5" />{t('label')}
        </div>
        <h2 className="text-2xl md:text-3xl font-bold text-primary tracking-tight">{t('title')}</h2>
        <p className="mt-2 text-secondary text-[14.5px]">{t('subtitle')}</p>
        <form onSubmit={submit} className="mt-5 flex gap-2 max-w-md">
          <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder={t('placeholder')} className="flex-1 px-4 py-2.5 rounded-lg bg-app border border-app text-primary placeholder-tertiary focus:border-[#F5C518]/40 focus:outline-none focus:ring-1 focus:ring-[#F5C518]/30 text-sm" />
