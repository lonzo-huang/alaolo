...
export function Comments({ slug }) {
  ...
  return (
    <section className="mt-12">
      <h2 className="text-lg font-bold text-primary mb-4 flex items-center gap-2"><MessageSquare className="w-4 h-4 text-[#F5C518]" />{t('comments')} ({list.length})</h2>
      <form onSubmit={submit} className="p-5 rounded-xl border border-app bg-surface">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <input value={name} onChange={e => setName(e.target.value)} placeholder={t('commentName')} maxLength={80} className="px-3 py-2 rounded-lg bg-app border border-app text-primary placeholder-tertiary text-sm focus:border-[#F5C518]/40 focus:outline-none" required />
          <div className="flex items-center gap-1 md:col-span-2">
            <span className="text-xs text-secondary mr-1">{t('commentRating')}:</span>
            {[1,2,3,4,5].map(n => (
        ...
        <textarea value={content} onChange={e => setContent(e.target.value)} placeholder={t('commentPlaceholder')} maxLength={2000} rows={3} className="w-full px-3 py-2 rounded-lg bg-app border border-app text-primary placeholder-tertiary text-sm focus:border-[#F5C518]/40 focus:outline-none resize-none" required />
        <div className="mt-3 flex items-center justify-between">
          ...
          <button disabled={loading} className="px-4 py-2 rounded-lg bg-[#F5C518] hover:bg-[#e6b800] text-black text-sm font-medium inline-flex items-center gap-1.5 disabled:opacity-50">
            <Send className="w-3.5 h-3.5" />{t('commentSubmit')}
          </button>
        </div>
      </form>
      ...
      <div className="mt-6 space-y-3">
        {list.length === 0 ? (
          <div className="text-center py-10 text-tertiary text-sm border border-dashed border-app rounded-xl">{t('commentEmpty')}</div>
        ) : list.map(c => (
