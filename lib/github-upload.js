// Commits a file into a DEDICATED media repo (separate from the app's code
// repo) via the GitHub Contents API. This keeps the code repo lean while
// jsDelivr's GitHub CDN (cdn.jsdelivr.net/gh/...) serves the media instantly
// (using the commit SHA bypasses jsDelivr's branch cache).
//
...
//
// Setup:
//   1. Create a new empty PUBLIC GitHub repo, e.g. "alaolo-media"
//   2. Generate a fine-grained PAT scoped ONLY to that repo (Contents: Read and write)
//   3. Set GITHUB_MEDIA_TOKEN / GITHUB_MEDIA_OWNER / GITHUB_MEDIA_REPO below

const GITHUB_API = 'https://api.github.com'

function getConfig() {
  const token = process.env.GITHUB_MEDIA_TOKEN
  const owner = process.env.GITHUB_MEDIA_OWNER
  const repo = process.env.GITHUB_MEDIA_REPO
  const branch = process.env.GITHUB_MEDIA_BRANCH || 'main'
  if (!token || !owner || !repo) {
    throw new Error('GITHUB_MEDIA_TOKEN, GITHUB_MEDIA_OWNER, GITHUB_MEDIA_REPO must be set in env (point these at your dedicated media repo, not the code repo)')
  }
  return { token, owner, repo, branch }
}

/**
 * @param {string} relativePath - path within the media repo, e.g. "images/tools/chatgpt.png"
 * @param {Buffer} buffer - file content
 * @param {string} [message] - commit message
 */
export async function commitFileToGitHub(relativePath, buffer, message) {
  const { token, owner, repo, branch } = getConfig()
  const fullPath = relativePath.replace(/^\/+/, '').replace(/\/+/g, '/')
